...
module fluxo_dados (
  ...
  input        reset,
    ...
    output [7:0] palavra_base
);
    // sinais internos para interligacao dos componentes
     wire   [7:0] s_endereco;
   wire   [6:0] s_letra;
   wire   [6:0] s_jogada;
     wire   [5:0] seletor_rom_contagem;

     wire   [7:0] seletor_palavra;
     wire   [7:0] s_ponteiros_rom;
	wire   [3:0] db_erros_wire;
    wire   [5:0] seletor_rom_reg;
  wire   [6:0] s_jogada_parcial;
  wire         tem_jogada_wire;
  wire         atualiza_jogada_parcial_wire;
  wire   [6:0] s_jogada_decodificada;
   // Contadores
	 // Contador para aleatorizar escolha da ROM de ponteiros
   contador_m #(.M(40), .N(6)) contador_rom (
    .clock	(clock),
    .zera_as(reset),
    .zera_s	(1'b0),
    .conta	(1'b1),
    ...
    contador_163 #(.N(8)) endereco_palavra (
      .clock	(clock),
      .clr(~zera_contador),
      .ld   (~load_end_inicial),
      .ent  (1'b1),
    ...
    registrador_n #(.N(8)) registrador_palavra (
      .clock	(clock),
      .clear	(zeraSeletor),
  ...
  registrador_n #(.N(6)) registrador_rom (
    .clock	(clock),
    .clear	(1'b0),
    .enable	(enable_rom),
  ...
  //registrador do modo escolhido
    ...
    assign db_endereco_letra = s_endereco[3:0];
    ...
    assign db_rom_escolhida = seletor_rom_reg[3:0];
