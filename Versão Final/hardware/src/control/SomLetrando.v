	...
	//  Wires
		 ...
		 debounce #(.tempo_debounce(DEBOUNCE_8MS_TICKS)) db_botao_7 (.clk(clock), .reset(1'b0), .entrada(reset_entrada), .saida(reset));
    ...
    tx_serial_7N2 transmissao_FPGA_inicial ( 
        ...
        .db_estado       () //ja esta em hexa; ser?? 4 display
    );
