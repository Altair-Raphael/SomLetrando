...
module unidade_controle  (
    ...
    output reg      errou,
    output reg      evento_reset_serial,
    output reg      evento_jogar_serial,
    output reg      evento_modo_serial,
    output reg      evento_erro_serial,
    output reg      pronto,
    ...
    always @* begin
        case (Eatual)
            ...
            espera_contador: Eprox = modo_transmissao ? envia_letra : espera;

            // Desvio para transmissao serial
            envia_letra:     Eprox = tx_start_ack ? espera_tx : envia_letra;
            // Se a letra transmitida foi o terminador, finaliza o modo de transmissao.
            espera_tx:       Eprox = tx_done ? (fim_palavra ? espera_contador : incrementa_reg) : espera_tx;
    ...
    always @* begin
        ...
        evento_reset_serial  = 1'b0;
        evento_jogar_serial  = (Eatual == preparacao) ? 1'b1 : 1'b0;
        evento_modo_serial   = (Eatual == load_rom) ? 1'b1 : 1'b0;
        evento_erro_serial   = (Eatual == registra) ? 1'b1 : 1'b0;
        case (Eatual)
