...
from PyQt5.QtWidgets import (
    ...
    QLabel, QPushButton, QComboBox, QSpinBox, QStackedWidget, QStatusBar, QMessageBox, QProgressBar
)
                        ...
                        if byte == 0:
                            ...
                            if mensagem:
                                if len(mensagem) > 1:
                                    ...
                                    self.falar_texto(f"A letra escolhida é: {mensagem}")
                            continue

                        ch = chr(byte)

                        # outros sinais (inicio, modo, confirmar e reset)
                        if ch == '*':
...
class TelaJogo(QMainWindow):
    ...
    def __init__(self):
        ...
        self.letras_recebidas_pendentes = []
    ...
    def processa_letra(self, letra_recebida: str):
        ...
        letra = letra_recebida.upper().strip()
        if not letra:
            return

        # Mitigacao: acumula letras recebidas entre confirmacoes para evitar
        # sobrescrita silenciosa por eventos de ajuda.
        self.letras_recebidas_pendentes.append(letra)
        self.letra_pendente = self.letras_recebidas_pendentes[0]

        if len(self.letras_recebidas_pendentes) > 1:
            self.label_mensagem.setText("⚠ Entrada ambígua detectada. Use Confirmar novamente.")
            self.label_mensagem.setStyleSheet("color: #b26a00;")
    ...
    def confirmar_tentativa(self):
        ...
        if not self.letras_recebidas_pendentes:
            self.letra_pendente = ""
            return

        if len(self.letras_recebidas_pendentes) > 1:
            self.letras_recebidas_pendentes.clear()
            self.letra_pendente = ""
            self.label_mensagem.setText("⚠ Confirmação ambígua: repita a jogada.")
            self.label_mensagem.setStyleSheet("color: #b26a00;")
            self.statusBar_widget.showMessage("Confirmação ambígua detectada. Nenhum avanço local aplicado.")
            return

        self.letra_pendente = self.letras_recebidas_pendentes[-1]
        ...
        if self.letra_pendente == letra_esperada:
            ...
            if self.contador_erros >= self.max_erros:
                self.em_derrota()
                QTimer.singleShot(500, lambda: self.stack.CurrentIndex(3))
    ...
    def troca_modo_jogo(self, modo: int):
        ...
        # Atualiza o texto na tela inicial para o usuário ver a mudança
        self.label_status_inicio.setText(f"Modo Selecionado: {self.modo_jogo}\nAguardando início...")
